  ...
  describe 'instance methods' do
    ...
    describe '#searched_movies' do
      it 'returns movies with the select word in its title', :vcr do
        expect(described_class.new('princess').searched_movies[0].title).to eq('The Princess')
        expect(described_class.new('princess').searched_movies[1].title).to eq('The Princess')
      end
    end

    describe '#movie_by_id' do
      it 'returns detales for the select movie by id', :vcr do
        expect(described_class.new(550).movie_by_id.title).to eq('Fight Club')
      end
    end

    describe '#cast' do
      it 'returns detales for the select movie by id', :vcr do
        expect(described_class.new(550).cast[0]).to eq('Edward Norton as The Narrator')
      end
    end
    ...
    describe '#reviews' do
      it 'returns detales for the select movie by id', :vcr do
        ...
        expect(described_class.new(550).reviews[0]).to eq(test_string)
      end
    end
  end
end
